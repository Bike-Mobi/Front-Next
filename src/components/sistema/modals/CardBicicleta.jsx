  ...
  return (
    <div className={`flex flex-col w-[300px] md:w-[350px] xl:w-[450px] h-[150px] md:h-full xl:h-full p-4 md:p-0 border border-cinza rounded-lg justify-between`}>
      <div className={`flex w-full`}>
        <img src={data?.photo ? data?.photo : '/Bike.jpg'} alt="image" className={`w-14 md:w-36 xl:w-40 h-14 md:h-36 xl:h-40 md:rounded-r-none rounded-md mr-4 object-cover hidden md:block`}/>
        <div className={`flex flex-col md:justify-around xl:justify-around justify-between w-full`}>
          <div className='flex mb-4'>
            <img src={data?.photo ? data?.photo : '/Bike.jpg'} alt="image" className={`w-14 md:w-28 xl:w-40 h-14 md:h-28 xl:h-40 md:rounded-r-none xl:rounded-r-none rounded-md mr-4 object-cover block md:hidden`}/>
            <div className='flex flex-col'>
              <span className={`font-semibold text-xl text-tomEscuro max-w-[194px] xl:max-w-full max-h-10 overflow-hidden break-all leading-none mt-[-1px]`}>{data?.name}</span>
